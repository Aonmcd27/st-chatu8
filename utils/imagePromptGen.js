 ...
 * 3. 如果没有找到 <images>/<image> 结构，回退到配置的标签格式
 * 
...
function extractImagePrompt(text) {
    ...
    if (imagesBlockMatch && imagesBlockMatch[1]) {
        ...
        if (allImageMatches.length > 0) {
            ...
            console.log('[imagePromptGen] Using last <image> tag (index ' + (allImageMatches.length - 1) + ')');
            ...
            if (innerMatches && innerMatches.length > 0) {
                ...
                if (prompt) {
                    console.log(`[imagePromptGen] Extracted ${startTag}...${endTag} from last <image> (last match):`, prompt.substring(0, 50) + '...');
                    return prompt;
                }
            }
            ...
            if (fallbackPrompt) {
                console.log('[imagePromptGen] Using last <image> content directly:', fallbackPrompt.substring(0, 50) + '...');
    ...
    if (legacyMatches && legacyMatches.length > 0) {
        console.log(`[imagePromptGen] Using legacy ${startTag}...${endTag} format (last match)`);
...
export async function handleImagePromptGenerate(userRequirement, userImages = []) {
    ...
    try {
        ...
        // 1. 先合并相邻相同角色的消息
        prompt = mergeAdjacentMessages(prompt, getMergeOptionsForRequestType('char_display'));
        ...
        // 5. 执行 LLM 请求
        const llmOutput = await LLM_CHAR_DISPLAY(prompt, { timeoutMs: 600000 });
