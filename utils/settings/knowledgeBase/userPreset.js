// @ts-nocheck
/**
 * userPreset.js - User人设预设管理模块
 * 与人设共享同一个预设池（personaProfiles.presets），
 * 但独立跟踪 currentUserPresetId、userEnabled、userInjectionMode
 */
...
/**
 * 确保 personaProfiles 数据结构中包含 user 相关字段
 */
function ensureDataStructure() {
    ...
    // 确保 user 字段存在
    if (settings.personaProfiles.currentUserPresetId === undefined) {
        settings.personaProfiles.currentUserPresetId = '';
    }
    if (settings.personaProfiles.userEnabled === undefined) {
        settings.personaProfiles.userEnabled = false;
    }
    if (settings.personaProfiles.userInjectionMode === undefined) {
        settings.personaProfiles.userInjectionMode = 'alwaysOn';
    }
    return settings.personaProfiles;
}

/**
 * 设置 User 控件 - 绑定所有事件（初始化时调用一次）
 * @param {jQuery} container - 设置面板的 jQuery 对象
 */
export function setupUserControls(container) {
    $container = container;
    ensureDataStructure();
    ...
    // 绑定预设选择
    container.find('#ch-user-preset-id').on('change', onPresetChange);

    // 绑定按钮
    container.find('#ch-user-new').on('click', onNewPreset);
    container.find('#ch-user-rename').on('click', onRenamePreset);
    container.find('#ch-user-save').on('click', onSavePreset);
    container.find('#ch-user-save-as').on('click', onSaveAsPreset);
    container.find('#ch-user-export').on('click', onExportPreset);
    container.find('#ch-user-export-all').on('click', onExportAllPresets);
    container.find('#ch-user-import').on('click', () => {
        container.find('#ch-user-import-file').click();
    });
    container.find('#ch-user-import-file').on('change', onImportPreset);
    container.find('#ch-user-delete').on('click', onDeletePreset);

    // 绑定头像上传
    container.find('#ch-user-avatar-upload-btn').on('click', () => {
        container.find('#ch-user-avatar-input').click();
    });
    container.find('#ch-user-avatar-input').on('change', onUserAvatarUpload);
    container.find('#ch-user-avatar-remove-btn').on('click', onUserAvatarRemove);
    container.find('#ch-user-avatar-preview').on('click', () => {
        container.find('#ch-user-avatar-input').click();
    });

    // 绑定全局控制
    container.find('#ch-user-enabled').on('change', onGlobalSettingChange);
...
/**
 * 刷新预设下拉列表（共享 personaProfiles.presets）
 */
export function loadUserPresetList() {
    const profiles = ensureDataStructure();
    const $select = $container?.find('#ch-user-preset-id');
    if (!$select || !$select.length) return;

    $select.empty();
    $select.append('<option value="">(请选择User预设)</option>');
    ...
    // 恢复选择
    if (profiles.currentUserPresetId && profiles.presets[profiles.currentUserPresetId]) {
        $select.val(profiles.currentUserPresetId);
    }
}
 ...
 * 加载当前选中预设的数据到 UI
 */
export async function loadUserPresetData() {
    const profiles = ensureDataStructure();
    const presetId = profiles.currentUserPresetId;
    const preset = presetId ? profiles.presets[presetId] : null;

    PERSONA_FIELDS.forEach(field => {
        if (field === 'avatarId') return; // 跳过 avatarId
        const $el = $container?.find(`#ch-user-${field}`);
        if ($el && $el.length) {
    ...
    if (preset && preset.avatarId) {
        try {
            ...
            if (base64) {
                displayUserAvatar(base64);
            } else {
                clearUserAvatarDisplay();
            }
        } catch (error) {
            console.error('[UserPreset] 加载头像失败:', error);
            clearUserAvatarDisplay();
        }
    } else {
        clearUserAvatarDisplay();
...
function loadGlobalSettings() {
    const profiles = ensureDataStructure();
    $container?.find('#ch-user-enabled').prop('checked', profiles.userEnabled || false);
}
...
async function onPresetChange() {
    const profiles = ensureDataStructure();
    const newPresetId = $container?.find('#ch-user-preset-id').val() || '';
...
async function onNewPreset() {
    ...
    const result = await stylInput('请输入新User预设的名称', '');
    ...
    if (profiles.presets[name]) {
        toastr.warning(`预设「${name}」已存在，请使用其他名称。`);
        return;
    }
...
function onSavePreset() {
    ...
    if (!presetId || !profiles.presets[presetId]) {
        toastr.warning('没有活动的User预设可保存。请先新建或选择一个预设。');
        return;
    }

    saveCurrentUserData(presetId);
    toastr.success(`User预设「${presetId}」已保存。`);
}

/** 另存为 */
async function onSaveAsPreset() {
    const profiles = ensureDataStructure();
    const defaultName = profiles.currentUserPresetId || '';

    const result = await stylInput('请输入新User预设的名称', defaultName);
    if (!result || !result.trim()) return;
...
async function onRenamePreset() {
    const profiles = ensureDataStructure();
    const currentName = profiles.currentUserPresetId;
    if (!currentName || !profiles.presets[currentName]) {
        toastr.warning('没有活动的User预设可重命名。');
        return;
    }
    
    // Check if it's a default preset that shouldn't be renamed, though there doesn't seem to be a specific default preset constraint in the code for userPresets, we add standard default protection
    if (currentName === "默认" || currentName === "default") {
        toastr.warning("默认预设不能重命名。");
        return;
    }
    
    const result = await stylInput("请输入新的User预设名称", currentName);
    ...
    // 如果人设预设也在使用这个名称，也一并更新？
    // 根据需求，共享池的情况下，更名相当于新建和删除。
    // 如果 profiles.currentPresetId 也是这个名称，可以一并更新
...
async function onDeletePreset() {
    ...
    if (!presetId || !profiles.presets[presetId]) {
        toastr.warning('没有选中的User预设可删除。');
        return;
    }

    const confirmed = await stylishConfirm(`确定要删除预设「${presetId}」吗？\n此操作不可撤销！\n注意：如果人设管理也在使用此预设，该预设也会被删除。`);
    if (!confirmed) return;
    ...
    if (avatarId) {
        try {
            ...
            console.error('[UserPreset] 删除头像失败:', error);
        }
    }
...
async function onExportPreset() {
    ...
    downloadJson(exportData, `st-chatu8-user-${presetId}.json`);
    ...
    for (const [presetId, preset] of Object.entries(profiles.presets)) {
        if (preset.avatarId) {
            try {
                ...
                console.error(`[UserPreset] 导出头像失败 (${presetId}):`, error);
            }
        }
    }

    downloadJson(exportData, 'st-chatu8-user-全部.json');
...
async function onImportPreset(event) {
    ...
    try {
            ...
            if (avatarsToImport[key]) {
                try {
                    const avatarId = await saveConfigImage(avatarsToImport[key], {
                        format: 'png',
                        filename: `user_avatar_${key}_${Date.now()}`
                    });
                    presetData.avatarId = avatarId;
                } catch (error) {
                    console.error('[UserPreset] 导入头像失败:', error);
                    presetData.avatarId = '';
                }
            }
        ...
        toastr.success(`成功导入 ${total} 个预设（${newCount} 个新增，${updateCount} 个更新）。`);
    } catch (e) {
        console.error('[UserPreset] 导入失败:', e);
...
function onGlobalSettingChange() {
    const profiles = ensureDataStructure();
    profiles.userEnabled = $container?.find('#ch-user-enabled').prop('checked') || false;
    saveSettingsDebounced();
}
...
function bindFieldAutoSave(container) {
    PERSONA_FIELDS.forEach(field => {
        container.find(`#ch-user-${field}`).on('input', () => {
            triggerAutoSave();
        });
    });
}
...
function triggerAutoSave() {
    const profiles = ensureDataStructure();
    if (!profiles.currentUserPresetId) return;

    clearTimeout(_autoSaveTimer);
    _autoSaveTimer = setTimeout(() => {
        if (!profiles.currentUserPresetId) return;
        saveCurrentUserData(profiles.currentUserPresetId);
    }, AUTO_SAVE_DELAY);
}
...
function clearUserFields() {
    PERSONA_FIELDS.forEach(field => {
        ...
        const el = document.getElementById(`ch-user-${field}`);
        if (el) {
            el.value = '';
        }
    });
...
/**
 * 从 UI 收集字段值并保存到指定预设（共享预设池）
 * @param {string} presetId
 */
function saveCurrentUserData(presetId) {
    const profiles = ensureDataStructure();
    const preset = {};
 ...
 * 头像上传处理
 */
async function onUserAvatarUpload(event) {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    try {
        ...
        console.error('[UserPreset] 头像上传失败:', error);
 ...
 * 移除头像
 */
async function onUserAvatarRemove() {
    const profiles = ensureDataStructure();
    const presetId = profiles.currentUserPresetId;
    ...
    try {
        ...
        console.error('[UserPreset] 移除头像失败:', error);
 ...
 * 显示头像
 */
function displayUserAvatar(base64) {
    const $preview = $container?.find('#ch-user-avatar-preview');
    const $img = $container?.find('#ch-user-avatar-img');
    const $removeBtn = $container?.find('#ch-user-avatar-remove-btn');
 ...
 * 导出头像获取函数供外部使用
 */
export async function getUserAvatarById(presetId) {
    const profiles = ensureDataStructure();
    ...
    try {
        ...
        console.error('[UserPreset] 获取头像失败:', error);
        return null;
    }
}
